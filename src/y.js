...
const App = () => {
  ...
  useEffect(() => {
    const createStreamingToken = async () => {
      try {
        ...
        setDebug('Error creating streaming token');
      }
    };
  ...
  const startStreaming = async () => {
    try {
      ...
      // const response = await avatarApi.current.createStartAvatar({ newSessionRequest: requestBody });


      // setSessionInfo(res);
      console.log({ sessionId })

      await startStreamingSession(sessionId);
    } catch (error) {
  ...
  const startStreamingSession = async (sessionId) => {
    try {
      const sdp = await createNewSession(avatarId, voiceId);
      const requestBody = {
      ...
      // Retry mechanism for session state errors
      const maxRetries = 5;
      let attempt = 0;
      let success = false;
      ...
      while (attempt < maxRetries && !success) {
        try {
          ...
          setDebug(`Attempt ${attempt} failed: ${err.message}`);
      ...
      if (!success) {
        throw new Error('Failed to start streaming session after multiple attempts');
      }
    } catch (error) {
  ...
  const stopAllSessions = async () => {
    try {
      ...
      if (Array.isArray(sessions.data.sessions)) {
        ...
        console.error('No sessions found to stop');
      }
    } catch (error) {
